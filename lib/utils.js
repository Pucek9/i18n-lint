...
var angularBlocks = ['let', 'if', 'else',
  'for', 'empty', 'switch', 'case', 'default',
...
module.exports = {
  ...
  isHardcodedString: function(str) {
    ...
    if (!str || this.isAngularNewSyntax(str)) {
      return false;
    }
  ...
  /**
   * Check a new Angular control flow syntax.
   *
   * @param {string} str.
   *
   * @returns {boolean} Returns the verification result.
   */
  isAngularNewSyntax: function(str) {
    var trimmedStr = str.startsWith('}') ? str.replace(/}\s*@/, '@') : str;
    if(!trimmedStr.startsWith('@')) {
      return false;
    }

    return angularBlocks.some(
      function (block) {
        return trimmedStr.startsWith('@' + block);
      }
    );
  }
};
